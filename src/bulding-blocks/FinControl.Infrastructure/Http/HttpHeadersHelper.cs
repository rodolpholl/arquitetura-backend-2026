...
public static class HttpHeadersHelper
{
    ...
    public static Guid ExtrairOuGerarGuid(
    ...
    public static Guid ExtrairCorrelationId(
        ...
        => ExtrairOuGerarGuid(headers, headerName, generateIfMissing: true);
    ...
    public static string? ExtrairHeader(IHeaderDictionary headers, string headerName)
    {
        if (headers == null || string.IsNullOrWhiteSpace(headerName))
            return null;
    ...
    /// <returns>Valor do header ou valor padrão.</returns>
    public static string ExtrairHeaderOuPadrão(
        IHeaderDictionary headers,
        string headerName,
        string defaultValue)
        => ExtrairHeader(headers, headerName) ?? defaultValue;
}
