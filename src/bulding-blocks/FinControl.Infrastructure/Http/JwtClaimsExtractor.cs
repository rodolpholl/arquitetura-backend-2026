...
public static class JwtClaimsExtractor
{
    ...
    public static (string Id, string Nome, string Email) ExtrairDadosUsuario(ClaimsPrincipal user)
    {
        if (user?.Identity?.IsAuthenticated != true)
            throw new InvalidOperationException("Usuário não autenticado.");

        var id = ExtrairClaimComFallback(
            user,
            "sub",
        ...
        var nome = ExtrairClaimComFallback(
            user,
            "name",
            ClaimTypes.Name,
            "preferred_username",
        ...
        var email = ExtrairClaimComFallback(
            user,
            "email",
            ClaimTypes.Email)
            ?? throw new InvalidOperationException(
    ...
    public static string? ExtrairClaimComFallback(ClaimsPrincipal user, params string[] claimNames)
    {
    ...
    /// <returns>Valor da claim ou null.</returns>
    public static string? ExtrairClaim(ClaimsPrincipal user, string claimType)
    ...
    /// <returns>Valor da claim ou valor padrão.</returns>
    public static string ExtrairClaimOuPadrão(ClaimsPrincipal user, string claimType, string defaultValue)
