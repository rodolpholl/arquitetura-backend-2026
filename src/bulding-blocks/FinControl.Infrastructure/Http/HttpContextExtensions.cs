...
public static class HttpContextExtensions
{
    ...
    public static (string Id, string Nome, string Email) ExtrairDadosUsuario(this HttpContext httpContext)
    {
        if (httpContext?.User == null)
            throw new InvalidOperationException("HttpContext.User é nulo.");

        return JwtClaimsExtractor.ExtrairDadosUsuario(httpContext.User);
    }
    ...
    public static Guid ExtrairCorrelationId(
        ...
        string headerName = "x-correlation-id")
    {
        ...
        return HttpHeadersHelper.ExtrairCorrelationId(httpContext.Request.Headers, headerName);
    }
    ...
    public static Guid ExtrairIdempotencyKey(
        this HttpContext httpContext,
        string headerName = "idempotency-key")
    {
        ...
        return HttpHeadersHelper.ExtrairIdempotencyKey(httpContext.Request.Headers, headerName);
    }
    ...
    /// <returns>Valor do header ou null.</returns>
    public static string? ExtrairHeader(this HttpContext httpContext, string headerName)
    {
        if (httpContext?.Request?.Headers == null)
            return null;

        return HttpHeadersHelper.ExtrairHeader(httpContext.Request.Headers, headerName);
    }
    ...
    /// <param name="httpContext">Contexto HTTP.</param>
    /// <param name="headerName">Nome do header.</param>
    /// <param name="headerValue">Valor do header.</param>
    public static void AdicionarHeaderResposta(
        this HttpContext httpContext,
        string headerName,
        string headerValue)
    {
    ...
    public static void AdicionarHeadersRastreamento(
        this HttpContext httpContext,
        Guid correlationId,
        Guid idempotencyKey)
    {
