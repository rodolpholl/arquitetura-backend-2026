...
namespace FinControl.Consolidado.Tests.Features;
...
public class ConsolidadoRegrasDenegocioTests
{
    ...
    private static (AtualizarSaldoConsolidadoCommandHandler command, GetSaldoConsolidadoQueryHandler query)
        CriarHandlers(CacheEmMemoria? cache = null)
    {
        ...
        var commandHandler = new AtualizarSaldoConsolidadoCommandHandler(
            cacheService,
            LockPassthrough().Object,
            NullLogger<AtualizarSaldoConsolidadoCommandHandler>.Instance);

        var queryHandler = new GetSaldoConsolidadoQueryHandler(
            cacheService,
    ...
    public async Task Comerciante_Sem_Lancamentos_Tem_Saldo_Acumulado_Zero()
    {
        ...
        var saldo = await query.Handle(new GetSaldoConsolidadoQuery(), CancellationToken.None);

        saldo.Saldo.Should().Be(0);
        saldo.SaldoDecimal.Should().Be(0m);
    }
    ...
    public async Task Comerciante_Sem_Lancamentos_Tem_Saldo_Por_Data_Zero()
    {
        ...
        var saldo = await query.Handle(new GetSaldoConsolidadoQuery(data), CancellationToken.None);
    ...
    public async Task Credito_Aumenta_Saldo_Consolidado_Do_Dia()
    {
        ...
        await command.Handle(new AtualizarSaldoConsolidadoCommand(15000, DataPadrao));

        var data = DateOnly.FromDateTime(DataPadrao.UtcDateTime);
        var saldo = await query.Handle(new GetSaldoConsolidadoQuery(data), CancellationToken.None);

        saldo.Saldo.Should().Be(15000);
        saldo.SaldoDecimal.Should().Be(150.00m);
    }
    ...
    public async Task Multiplos_Creditos_Acumulam_No_Saldo_Do_Dia()
    {
        ...
        var (command, query) = CriarHandlers();
        await command.Handle(new AtualizarSaldoConsolidadoCommand(10000, DataPadrao));
        await command.Handle(new AtualizarSaldoConsolidadoCommand(5000, DataPadrao));
    ...
    public async Task Debito_Reduz_Saldo_Consolidado_Do_Dia()
    {
        ...
        var (command, query) = CriarHandlers();
        await command.Handle(new AtualizarSaldoConsolidadoCommand(20000, DataPadrao));
        await command.Handle(new AtualizarSaldoConsolidadoCommand(-8000, DataPadrao));

        var data = DateOnly.FromDateTime(DataPadrao.UtcDateTime);
        var saldo = await query.Handle(new GetSaldoConsolidadoQuery(data), CancellationToken.None);

        saldo.SaldoDecimal.Should().Be(120.00m);
    }
    ...
    public async Task Debitos_Superiores_Aos_Creditos_Resultam_Em_Saldo_Negativo()
    {
        ...
        saldo.Saldo.Should().BeNegative();
        saldo.SaldoDecimal.Should().Be(-30.00m);
    }
    ...
    public async Task Saldo_Acumulado_Reflete_Total_De_Todos_Os_Lancamentos()
    {
        ...
        await command.Handle(new AtualizarSaldoConsolidadoCommand(10000, dia1));
        await command.Handle(new AtualizarSaldoConsolidadoCommand(20000, dia2));

        var saldo = await query.Handle(new GetSaldoConsolidadoQuery(), CancellationToken.None);

        saldo.SaldoDecimal.Should().Be(300.00m);
    }
    ...
    public async Task Saldo_Acumulado_E_Saldo_Diario_Sao_Consultas_Independentes()
    {
        ...
        await command.Handle(new AtualizarSaldoConsolidadoCommand(10000, dia1));
        await command.Handle(new AtualizarSaldoConsolidadoCommand(5000, dia2));

        var saldoAcumulado = await query.Handle(new GetSaldoConsolidadoQuery(), CancellationToken.None);
        var saldoDia2 = await query.Handle(
            new GetSaldoConsolidadoQuery(DateOnly.FromDateTime(dia2.UtcDateTime)),
            CancellationToken.None);

        saldoAcumulado.SaldoDecimal.Should().Be(150.00m);
        // O saldo do dia 2 reflete o acumulado até aquele momento (100 + 50)
        saldoDia2.SaldoDecimal.Should().Be(150.00m);
    }
    ...
    public async Task Consulta_Por_Data_Retorna_Saldo_Do_Dia_Solicitado()
    {
        ...
        await command.Handle(new AtualizarSaldoConsolidadoCommand(7500, data));

        var saldo = await query.Handle(
            new GetSaldoConsolidadoQuery(new DateOnly(2026, 5, 20)),
            CancellationToken.None);

        saldo.SaldoDecimal.Should().Be(75.00m);
    }
    ...
    public async Task Consulta_De_Dia_Sem_Lancamentos_Retorna_Saldo_Mais_Recente_Disponivel()
    {
        ...
        await command.Handle(new AtualizarSaldoConsolidadoCommand(9000, dia22));

        var saldo = await query.Handle(
            new GetSaldoConsolidadoQuery(new DateOnly(2026, 5, 23)),
            CancellationToken.None);

        saldo.SaldoDecimal.Should().Be(90.00m);
    }
    ...
    public async Task Lancamento_Retroativo_E_Consolidado_Na_Data_Informada()
    {
        ...
        await command.Handle(new AtualizarSaldoConsolidadoCommand(5000, dataRetroativa));

        var saldo = await query.Handle(
            new GetSaldoConsolidadoQuery(new DateOnly(2025, 12, 31)),
            CancellationToken.None);

        saldo.SaldoDecimal.Should().Be(50.00m);
    }
    ...
    public async Task Valor_Em_Centavos_E_Convertido_Corretamente_Para_Reais()
    {
        ...
        var (command, query) = CriarHandlers();
        await command.Handle(new AtualizarSaldoConsolidadoCommand(123456, DataPadrao));

        var saldo = await query.Handle(new GetSaldoConsolidadoQuery(), CancellationToken.None);

        saldo.SaldoDecimal.Should().Be(1234.56m);
    }
}
