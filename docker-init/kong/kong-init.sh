...
curl -i -X POST http://kong:8001/services/agile-api/routes \
  --data "hosts[]=api.localhost" \
  --data "paths[]=/api" \
  --data name=agile-api-route || echo "Route já existe"

echo ""

# 3. Configurar OIDC Plugin no Route
echo "  3️⃣ Configurando OIDC Plugin..."
curl -i -X POST http://kong:8001/routes/agile-api-route/plugins \
  --data name=oidc \
  --data config.client_id=kong-client \
  --data config.client_secret=kong-secret \
  --data config.discovery="http://keycloak:8080/realms/agile/.well-known/openid-configuration" \
  --data config.redirect_uri="http://localhost:8000/api/" \
  --data config.scope="openid profile email" \
  --data config.session_secret="agile-kong-session-secret-12345" \
...
echo "   2. Login: admin / agile_keycloak_password_123"
echo "   3. Criar realm 'agile' (ou use 'master')"
